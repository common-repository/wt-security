...
class WebTotemDB
{
    ...
    const WTOTEM_TABLE_CVE_LIST = 'wtotem_plugins_cve_list';
    ...
    public static function install()
    {
        ...
        if ($wpdb->get_var("show tables like '$audit_logs_table'") != $audit_logs_table) {
            ...
            $current_collation = $wpdb->get_var("SELECT TABLE_COLLATION FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '$audit_logs_table'");

            if ($current_collation != 'utf8_general_ci') {
                $wpdb->query("ALTER TABLE $audit_logs_table CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci ");
            }
        }
        ...
        $cve_list_table = self::add_prefix(self::WTOTEM_TABLE_CVE_LIST);
        if ($wpdb->get_var("show tables like '$cve_list_table'") != $cve_list_table) {

            $sql = "CREATE TABLE " . $cve_list_table . " (
                id bigint NOT NULL AUTO_INCREMENT,
                cve_id tinytext NOT NULL,
                plugin_name tinytext NOT NULL,
                plugin_version tinytext,
                slug tinytext,
                new_version tinytext,
                cve_data text,
                UNIQUE KEY id (id)
              )
            DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;";
    ...
    private static function getTable($name)
    {
        switch ($name) {
            ...
            case 'plugins_cve_list':
                return self::add_prefix(self::WTOTEM_TABLE_CVE_LIST);
