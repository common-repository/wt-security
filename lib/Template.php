...
class WebTotemTemplate
{
    ...
    public function twigRender($template, $variables = [])
    {
        ...
        $variables['user_email'] = WebTotem::getUserEmail();
    ...
    public function baseTemplate($page_content)
    {
        ...
        if ($page != 'activation') {
            ...
            if(is_array($user_feedback) and array_key_exists('score', $user_feedback)){
                $variables['user_feedback'] = (bool)$user_feedback['score'];
            } else {
                $variables['user_feedback'] = true;
            }
        } else {
            $variables['user_feedback'] = true;
        }
        ...
        $site_created_at = (array_key_exists($host['name'], $site_created_at)) ? $site_created_at[$host['name']] : null;
        $variables['site_created_at'] = $site_created_at ? date('Y-m-d', strtotime($site_created_at)) : null;
